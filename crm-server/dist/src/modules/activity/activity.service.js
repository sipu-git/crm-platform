...
export const activityService = {
    ...
    async create(tenantId, createdBy, data) {
        const activity = await prisma.$transaction(async (tx) => {
            ...
            if (!deal)
                throw ApiError.badRequest("Deal not found in this tenant");
            if (!contact)
                throw ApiError.badRequest("Contact not found in this tenant");
            if (!company)
                throw ApiError.badRequest("Company not found in this tenant");
            if (data.assignedTo) {
                const assignee = await assignRepository.viewAssignee(tx, tenantId, data.assignedTo);
                if (!assignee)
                    throw ApiError.badRequest("Assignee not found in this tenant");
            }
