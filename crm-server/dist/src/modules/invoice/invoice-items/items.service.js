import { invoiceItemsRepository } from "./invoice-items.repository.js";
import { computeInvoiceItemAmounts, recomputeInvoiceItemAmounts } from "./items-calculation.js";
import { ApiError } from "../../../shared/utils/ApiError.js";
import { invoiceRepository } from "../total-invoices/invoice.repository.js";
import { prisma } from "../../../../lib/prisma.js";
async function findInvoiceOrThrow(tx, tenantId, invoiceId) {
    const invoice = await invoiceRepository.findById(tx, tenantId, invoiceId);
    if (!invoice)
        throw ApiError.notFound("Invoice not found");
    return invoice;
}
async function recalculateInvoiceTotals(tx, tenantId, invoiceId) {
...
export const invoiceItemsService = {
    ...
    async create(tenantId, invoiceId, data) {
        return prisma.$transaction(async (tx) => {
    ...
    async update(tenantId, invoiceId, itemId, patch) {
        return prisma.$transaction(async (tx) => {
            const invoice = await findInvoiceOrThrow(tx, tenantId, invoiceId);
            if (invoice.status !== "DRAFT") {
    ...
    async delete(tenantId, invoiceId, itemId) {
        return prisma.$transaction(async (tx) => {
            const invoice = await findInvoiceOrThrow(tx, tenantId, invoiceId);
            if (invoice.status !== "DRAFT") {
