...
import { listInvoicesQuerySchema } from '../validations/invoice.schema.js';
...
export const invoiceController = {
  async list(req: Request, res: Response) {
    ...
    const invoices = await invoiceService.list(req.auth.tenantId, req.auth, parsed.status, parsed.dealId);
  ...
  async viewOwnInvoice(req: Request, res: Response) {
    if (!req.auth) throw ApiError.unauthorized('Not authenticated');
    const invoice = await invoiceService.viewOwnInvoice(req.auth.tenantId, req.auth.userId!);
    return res.status(200).json(successResponse("Invoice fetched successfully!", invoice));
  },
